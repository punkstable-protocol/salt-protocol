...
contract('MasterChef', ([alice, bob, carol, minter]) => {
    context('With ERC/LP token added to the field', () => {
        ...
        it('should give Tickets only after farming time', async () => {
            ...
            await this.testmasterChef.deposit(0, '0', { from: bob }); // block 90
            assert.equal((await this.testmasterChef.ticketBalanceOf(bob)).valueOf(), '0');
            await time.advanceBlockTo('94');
            await this.testmasterChef.deposit(0, '0', { from: bob }); // block 95
            assert.equal((await this.testmasterChef.ticketBalanceOf(bob)).valueOf(), '0');
            await time.advanceBlockTo('99');
            await this.testmasterChef.deposit(0, '0', { from: bob }); // block 100
            assert.equal((await this.testmasterChef.ticketBalanceOf(bob)).valueOf(), '0');
            await time.advanceBlockTo('100');
            await this.testmasterChef.deposit(0, '0', { from: bob }); // block 101
            assert.equal((await this.testmasterChef.ticketBalanceOf(bob)).valueOf(), '100');
            await time.advanceBlockTo('104');
            await this.testmasterChef.deposit(0, '0', { from: bob }); // block 105
            assert.equal((await this.testmasterChef.ticketBalanceOf(bob)).valueOf(), '500');
        });
        ...
        it('should not distribute Tickets if no one deposit', async () => {
            ...
            await this.testmasterChef.deposit(0, '10', { from: bob }); // block 210
            assert.equal((await this.testmasterChef.ticketBalanceOf(bob)).valueOf(), '0');
            assert.equal((await this.lp.balanceOf(bob)).valueOf(), '990');
            await time.advanceBlockTo('219');
            await this.testmasterChef.withdraw(0, '10', { from: bob }); // block 220
